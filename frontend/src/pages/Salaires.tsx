...
const Salaires = () => {
    ...
    for (const row of rows) {
      try {
        await api.post('/salaires/salaries/', {
          nom: row.nom || '',
          prenom: row.prenom || '',
          salaire_base: parseFloat(String(row.salaire_base).replace(',', '.')),
  ...
  const handleImportActions = async (rows: any[]) => {
    for (const row of rows) {
      try {
        // Trouver le salarié par nom
        const salarie = salaries.find(s =>
          `${s.nom} ${s.prenom}`.toLowerCase() === String(row.salarie || '').toLowerCase()
        )
        if (!salarie) continue
            ...
            {activeTab === 'etat' && (
              <SortableTable
                ...
                columns={[
                  ...
                  { key: 'statut', label: 'Statut', sortable: false, render: (_v: any, row: any) => (
            ...
            {activeTab === 'salaries' && (
              <div>
                ...
                <SortableTable
                  ...
                  columns={[
                    { key: 'periode_fmt', label: 'Période', sortable: false, render: (_v: any, row: any) => editingSalarieId === row.id ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}><span style={{ fontSize: '10px', color: '#888', width: '50px' }}>Début :</span><input type="date" value={editingDateDebut} onChange={e => setEditingDateDebut(e.target.value)} style={{ ...inputStyle, width: '140px', padding: '4px 8px' }} /></div>
                        <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}><span style={{ fontSize: '10px', color: '#888', width: '50px' }}>Fin :</span><input type="date" value={editingDateFin} onChange={e => setEditingDateFin(e.target.value)} style={{ ...inputStyle, width: '140px', padding: '4px 8px' }} /></div>
                        <div style={{ display: 'flex', gap: '4px', marginTop: '2px' }}><button onClick={() => handleUpdatePeriode(row.id)} style={btnOk}>✓ OK</button><button onClick={() => setEditingSalarieId(null)} style={btnCancel}>✕</button></div>
                      </div>
                    ) : (
                    ...
                    { key: 'actions', label: 'Actions', sortable: false, render: (_v: any, row: any) => <button onClick={() => handleDeleteSalarie(row.id)} style={{ padding: '4px 10px', background: '#fdeaea', color: '#c0392b', border: '1px solid #f5c6c6', borderRadius: '4px', fontSize: '11px', cursor: 'pointer' }}>Supprimer</button> },
                  ]}
                  data={salarieData}
                />
              </div>
              ...
              <div>
                ...
                <SortableTable
                  ...
                  columns={[
                    ...
                    { key: 'type_label', label: 'Type', render: (_v: any, row: any) => <span style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: '600', background: row.type === 'entree' ? '#e8f8ef' : '#fdeaea', color: row.type === 'entree' ? '#1a7a40' : '#c0392b' }}>{row.type === 'entree' ? '↑ Entrée' : '↓ Sortie'}</span> },
                    { key: 'categorie', label: 'Catégorie', render: (_v: any, row: any) => <span style={{ color: '#555' }}>{row.categorie || '—'}</span> },
                    { key: 'montant_fmt', label: 'Montant', sortable: false, render: (_v: any, row: any) => editingActionId === row.id ? (
                    ...
                    { key: 'date_fmt', label: 'Date', sortable: false, render: (_v: any, row: any) => editingActionId === row.id ? (
                    ...
                    { key: 'del', label: 'Actions', sortable: false, render: (_v: any, row: any) => <button onClick={() => handleDeleteAction(row.id)} style={{ padding: '4px 10px', background: '#fdeaea', color: '#c0392b', border: '1px solid #f5c6c6', borderRadius: '4px', fontSize: '11px', cursor: 'pointer' }}>Supprimer</button> },
                  ]}
                  data={actionsData}
                />
              </div>
