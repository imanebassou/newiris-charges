...
const Previsions = () => {
 ...
 const handleDelete = async (e: React.MouseEvent, id: number) => {
  ...
  const editInputStyle = {
    width: '38px', padding: '3px 4px', borderRadius: '4px',
    border: '1px solid rgba(255,255,255,0.4)', fontSize: '12px',
    textAlign: 'center' as const, color: '#fff', fontWeight: '700' as const,
    background: 'rgba(255,255,255,0.15)', outline: 'none',
  }

  const tabBtn = (active: boolean) => ({
    padding: '6px 16px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', fontWeight: '600' as const,
    background: active ? '#1a3a6b' : '#fff', color: active ? '#fff' : '#555',
  ...
  const renderCard = (prev: any) => {
    ...
    if (isEditing) {
      return (
        <div key={prev.id} style={{
          ...
          border: '2px solid #0099cc', borderLeft: `4px solid ${isEntree ? '#1a7a40' : '#c0392b'}`,
          marginBottom: '8px',
        }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div>
              <label style={{ fontSize: '11px', color: '#888', display: 'block', marginBottom: '2px' }}>Titre</label>
              <input style={smallInput} value={editForm.titre} onChange={e => setEditForm({ ...editForm, titre: e.target.value })} />
            </div>
            <div>
              <label style={{ fontSize: '11px', color: '#888', display: 'block', marginBottom: '2px' }}>Montant (DH)</label>
              <input style={smallInput} type="number" value={editForm.montant} onChange={e => setEditForm({ ...editForm, montant: e.target.value })} />
            </div>
            <div>
              <label style={{ fontSize: '11px', color: '#888', display: 'block', marginBottom: '2px' }}>Date (change la semaine auto)</label>
            ...
            <div>
              <label style={{ fontSize: '11px', color: '#888', display: 'block', marginBottom: '2px' }}>Catégorie</label>
            ...
            <div>
              <label style={{ fontSize: '11px', color: '#888', display: 'block', marginBottom: '2px' }}>Personne</label>
            ...
            <div>
              <label style={{ fontSize: '11px', color: '#888', display: 'block', marginBottom: '2px' }}>Statut</label>
    ...
    return (
      <div key={prev.id} style={{
        background: '#fff', borderRadius: '8px', padding: '12px 14px', marginBottom: '8px',
        border: '1px solid #e8eaed', borderLeft: `4px solid ${isEntree ? '#1a7a40' : '#c0392b'}`,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
          ...
          <div style={{ display: 'flex', gap: '4px' }}>
            <button onClick={e => {
              e.stopPropagation()
              setEditingCard(prev.id)
              setEditForm({
                titre: prev.titre, description: prev.description || '',
                montant: String(prev.montant), date_prevision: prev.date_prevision,
                categorie: prev.categorie || '', statut: prev.statut,
              })
            }} style={{ background: '#e8f4fb', border: '1px solid #b3d9f0', color: '#0099cc', fontSize: '11px', cursor: 'pointer', padding: '2px 8px', borderRadius: '4px' }}>✏️</button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            {editingSemaine === idx ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                }} style={{ padding: '3px 8px', background: '#0099cc', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '11px', cursor: 'pointer' }}>OK</button>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <button onClick={() => { setEditingSemaine(idx); setEditDebutFin({ debut: String(s.debut), fin: String(s.fin) }) }}
                  style={{ background: 'none', border: 'none', color: '#ccc', fontSize: '13px', cursor: 'pointer' }}>✏️</button>
              </div>
            )}
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: '10px', padding: '16px 20px', border: '1px solid #e8eaed', borderTop: `3px solid ${ecartSemaine.ecart >= 0 ? '#1a7a40' : '#c0392b'}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: '24px', fontWeight: '800', color: ecartSemaine.ecart >= 0 ? '#1a7a40' : '#c0392b' }}>
                  {ecartSemaine.ecart.toLocaleString('fr-FR')} DH
                </div>
                <div style={{ marginTop: '6px' }}>
                  <span style={{ fontSize: '10px', padding: '3px 8px', borderRadius: '4px', background: semaineInfo.bg, color: semaineInfo.color, fontWeight: '700' }}>{semaineInfo.label}</span>
                </div>
              </div>
                        ...
                        <div style={{ padding: '14px' }}>
                          {ecartSemaine ? (
                            <>
                              ...
                              <div style={{ fontSize: '20px', fontWeight: '800', color: ecartSemaine.ecart >= 0 ? '#1a7a40' : '#c0392b' }}>
                                {ecartSemaine.ecart.toLocaleString('fr-FR')} DH
                              </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                      <BarChart data={[1, 2, 3, 4].map(s => ({ name: `S${s}`, ecart: ecarts?.[`semaine_${s}`]?.ecart || 0 }))}>
                        ...
                        <Bar dataKey="ecart" radius={[4, 4, 0, 0]}>
                          {[1, 2, 3, 4].map(s => (
                            <Cell key={s} fill={(ecarts?.[`semaine_${s}`]?.ecart || 0) >= 0 ? '#1a7a40' : '#c0392b'} />
                          ))}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  ...
                  <div style={{ background: '#fff', borderRadius: '8px', padding: '16px', border: '1px solid #e8eaed' }}>
                    ...
                    {getPieData('entree').length > 0 ? (
                      <ResponsiveContainer width="100%" height={200}>
                        <PieChart>
                          <Pie data={getPieData('entree')} cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value"
                  ...
                  <div style={{ background: '#fff', borderRadius: '8px', padding: '16px', border: '1px solid #e8eaed' }}>
                    ...
                    {getPieData('sortie').length > 0 ? (
                      <ResponsiveContainer width="100%" height={200}>
                        <PieChart>
                          <Pie data={getPieData('sortie')} cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value"
                            label={({ name, percent }) => `${name} ${Math.round((percent || 0) * 100)}%`} labelLine={false}>
                        ...
                        <div>
                          ...
                          {showAddCat && (
                            <div style={addNewStyle}>
                              <input style={{ ...inputStyle, flex: 1 }} value={newCat} onChange={e => setNewCat(e.target.value)} placeholder="Nouvelle catégorie..." />
