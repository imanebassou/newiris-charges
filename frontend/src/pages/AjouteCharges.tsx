...
const AjouteCharges = () => {
  ...
  const [form, setForm] = useState({ titre: '', service: '', categorie: '', sous_categorie: '', montant: '', date: '', personne: '', statut: 'en_cours' })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPhoto(null); fetchData()
    } catch { setError('Erreur lors de la soumission. Vérifiez les champs.') }
  ...
  const tableData = charges.map(c => ({
    ...c,
    montant_fmt: `${parseFloat(c.montant).toLocaleString('fr-FR')} DH`,
    date_fmt: new Date(c.date).toLocaleDateString('fr-FR'),
    personne_label: c.description || '—',
    sous_categorie: c.sous_categorie || '—',
    statut_label: c.statut === 'traitee' ? 'Traitée' : 'En cours',
  }))
    ...
    <Layout>
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <button onClick={() => setShowForm(!showForm)} style={{ padding: '8px 16px', background: '#0099cc', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', fontWeight: '600' }}>+ Nouvelle charge</button>
        </div>
        ...
        {success && <div style={{ background: '#e8f8ef', border: '1px solid #a8d5b5', borderRadius: '6px', padding: '12px 16px', fontSize: '13px', color: '#1a7a40', marginBottom: '16px' }}>✓ Charge soumise avec succès !</div>}
          ...
          <div style={{ background: '#fff', borderRadius: '8px', padding: '24px', border: '1px solid #e8eaed', marginBottom: '20px' }}>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div><label style={{ fontSize: '11px', color: '#555', display: 'block', marginBottom: '4px' }}>Titre *</label><input style={inputStyle} value={form.titre} onChange={e => setForm({ ...form, titre: e.target.value })} required placeholder="Ex: Carburant véhicule" /></div>
                <div><label style={{ fontSize: '11px', color: '#555', display: 'block', marginBottom: '4px' }}>Service *</label><select style={inputStyle} value={form.service} onChange={e => setForm({ ...form, service: e.target.value })} required><option value="">Sélectionner un service...</option>{services.map(s => <option key={s.id} value={s.id}>{s.nom}</option>)}</select></div>
                <div>
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
                <button type="submit" disabled={submitting} style={{ padding: '8px 20px', background: submitting ? '#aaa' : '#1a3a6b', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '12px', cursor: submitting ? 'not-allowed' : 'pointer' }}>{submitting ? 'Envoi...' : 'Soumettre'}</button>
        ...
        {loading ? (
          ...
          <SortableTable
            ...
            columns={[
              ...
              { key: 'sous_categorie', label: 'Sous-catégorie', render: (_v: any, row: any) => <span style={{ color: '#555' }}>{row.sous_categorie}</span> },
              { key: 'montant_fmt', label: 'Montant', render: (_v: any, row: any) => <span style={{ fontWeight: '600', color: '#e84c3d' }}>{row.montant_fmt}</span> },
              { key: 'date_fmt', label: 'Date', render: (_v: any, row: any) => <span style={{ color: '#555' }}>{row.date_fmt}</span> },
              { key: 'personne_label', label: 'Personne', render: (_v: any, row: any) => <span style={{ color: '#555' }}>{row.personne_label}</span> },
              {
              ...
              },
              {
                ...
                render: (_v: any, row: any) => (
                  <select value={row.statut} onChange={async e => { try { await api.patch(`/charges-variables/${row.id}/`, { statut: e.target.value }); fetchData() } catch (err) { console.error(err) } }}
