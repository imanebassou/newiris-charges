...
const Caisse = () => {
  ...
  const addAction = async () => {
    if (!newAction.titre || !newAction.montant) {
      setError('Titre et montant sont obligatoires.')
      return
    }
    setError('')
    try {
      ...
      setNewAction({
        type: 'entree', titre: '', service: '', categorie: 'autre',
        montant: '', date: new Date().toISOString().split('T')[0],
        description: '', personne: '', photo: null,
      })
    ...
    <Layout>
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
          <button onClick={() => setActiveTab('principale')} style={{
            padding: '7px 16px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '12px',
            background: activeTab === 'principale' ? '#1a3a6b' : '#e8edf5',
            color: activeTab === 'principale' ? '#fff' : '#1a3a6b',
            fontWeight: activeTab === 'principale' ? 700 : 400,
          }}>Solde Caisse</button>

          {caisses.map(c => (
            <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              <button onClick={() => setActiveTab(c.id)} style={{
                padding: '7px 16px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '12px',
                background: activeTab === c.id ? '#1a3a6b' : '#e8edf5',
                color: activeTab === c.id ? '#fff' : '#1a3a6b',
                fontWeight: activeTab === c.id ? 700 : 400,
              }}>{c.nom}</button>
              <button onClick={() => deleteCaisse(c.id)} style={{
                background: '#e84c3d', color: '#fff', border: 'none',
          ...
          <button onClick={() => setShowAddCaisse(!showAddCaisse)} style={{
            padding: '7px 14px', background: '#fff', color: '#1a3a6b',
            border: '1px solid #1a3a6b', borderRadius: '6px', cursor: 'pointer', fontSize: '12px',
          }}>+ Caisse</button>
        </div>
        ...
        {showAddCaisse && (
          <div style={{ background: '#fff', borderRadius: '8px', padding: '16px', border: '1px solid #e8eaed', marginBottom: '16px', display: 'flex', gap: '12px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
            <div>
              ...
              <input style={{ ...inputStyle, width: '180px' }} value={newCaisse.nom}
                onChange={e => setNewCaisse(p => ({ ...p, nom: e.target.value }))} placeholder="Ex: Caisse Ahmed" />
            </div>
            <div>
              <label style={{ fontSize: '11px', color: '#555', display: 'block', marginBottom: '4px' }}>Solde initial (DH)</label>
              <input style={{ ...inputStyle, width: '140px' }} type="number" value={newCaisse.solde_initial}
                onChange={e => setNewCaisse(p => ({ ...p, solde_initial: e.target.value }))} placeholder="0.00" />
            </div>
        ...
        {activeTab === 'principale' ? (
          <div style={{ background: '#fff', borderRadius: '8px', padding: '16px', border: '1px solid #e8eaed', borderTop: '3px solid #1a3a6b', marginBottom: '20px', display: 'inline-block', minWidth: '220px' }}>
            ...
            {editSolde ? (
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginTop: '4px' }}>
                <input type="number" value={soldeInput} onChange={e => setSoldeInput(e.target.value)}
          ...
          <div style={{ background: '#fff', borderRadius: '8px', padding: '16px', border: '1px solid #e8eaed', borderTop: '3px solid #0099cc', marginBottom: '20px', display: 'inline-block', minWidth: '220px' }}>
            ...
            {editSoldePerso === currentCaisse.id ? (
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginTop: '4px' }}>
                <input type="number" value={soldePersoInput} onChange={e => setSoldePersoInput(e.target.value)}
                  style={{ ...inputStyle, width: '120px' }} />
              ...
              <>
                ...
                <button onClick={() => { setEditSoldePerso(currentCaisse.id); setSoldePersoInput(currentCaisse.solde_initial) }}
                  style={{ marginTop: '6px', padding: '3px 10px', background: 'none', border: '1px solid #0099cc', color: '#0099cc', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>✏️ Modifier solde initial</button>
              </>
            )}
          </div>
        )}
          ...
          <div style={{ background: '#fff', borderRadius: '8px', padding: '24px', border: '1px solid #e8eaed', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              ...
              <div>
                ...
                <input style={inputStyle} value={newAction.titre}
                  onChange={e => setNewAction(p => ({ ...p, titre: e.target.value }))} placeholder="Ex: Règlement fournisseur" />
              </div>
              ...
              <div>
                ...
                <input style={inputStyle} type="number" value={newAction.montant}
                  onChange={e => setNewAction(p => ({ ...p, montant: e.target.value }))} placeholder="0.00" />
              </div>

              <div>
                <label style={{ fontSize: '11px', color: '#555', display: 'block', marginBottom: '4px' }}>Date *</label>
                <input style={inputStyle} type="date" value={newAction.date}
              ...
              <div>
                ...
                <input style={inputStyle} value={newAction.description}
                  onChange={e => setNewAction(p => ({ ...p, description: e.target.value }))} placeholder="Description optionnelle" />
              </div>
              ...
              <div>
                ...
                {showAddPerson && (
                  <div style={addNewInputStyle}>
                    <input style={{ ...inputStyle, flex: 1 }} value={newPerson}
                      onChange={e => setNewPerson(e.target.value)} placeholder="Nom de la personne..."
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <label style={{
                  ...
                  background: newAction.photo ? '#e8f4fb' : '#fafafa',
                  borderColor: newAction.photo ? '#0099cc' : '#e0e0e0',
                }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '8px', background: newAction.photo ? '#0099cc' : '#e8eaed', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0, color: '#fff' }}>
                    {newAction.photo ? '✓' : '📎'}
                  </div>
                  <div>
                    <div style={{ fontSize: '12px', fontWeight: '600', color: newAction.photo ? '#0099cc' : '#555' }}>
                      {newAction.photo ? newAction.photo.name : 'Cliquer pour ajouter une photo'}
                    </div>
                    <div style={{ fontSize: '10px', color: '#aaa', marginTop: '2px' }}>
                      {newAction.photo ? `${(newAction.photo.size / 1024).toFixed(1)} KB` : 'JPG, PNG — max 5MB'}
                    </div>
                  </div>
                  <input type="file" accept="image/*" ref={fileRef}
                    onChange={e => setNewAction(p => ({ ...p, photo: e.target.files?.[0] || null }))}
                    style={{ display: 'none' }} />
                </label>
                {newAction.photo && (
                  <button type="button" onClick={() => { setNewAction(p => ({ ...p, photo: null })); if (fileRef.current) fileRef.current.value = '' }}
                    style={{ marginTop: '6px', padding: '3px 10px', background: '#fdeaea', color: '#c0392b', border: '1px solid #f5c6c6', borderRadius: '4px', fontSize: '11px', cursor: 'pointer' }}>
                    Supprimer la photo
                  </button>
                )}
              </div>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
              <button onClick={addAction} style={{ padding: '8px 20px', background: '#1a3a6b', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>
                Enregistrer
              </button>
              <button onClick={() => { setShowAddAction(false); setError('') }} style={{ padding: '8px 20px', background: '#fff', color: '#555', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>
                Annuler
              </button>
            </div>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
              <thead>
                <tr style={{ background: '#f8f9fa' }}>
                  {['#', 'Type', 'Titre', 'Service', 'Catégorie', 'Montant', 'Date', 'Personne', 'Photo', 'Statut', 'Description', 'Supprimer'].map(h => (
              ...
              <tbody>
                {currentActions.length === 0 ? (
                  <tr><td colSpan={12} style={{ textAlign: 'center', padding: '32px', color: '#aaa' }}>Aucune transaction</td></tr>
                ) : currentActions.map((a: any, i: number) => (
                  <tr key={a.id} style={{ borderBottom: '1px solid #f5f5f5' }}>
                    <td style={{ padding: '10px 14px', color: '#aaa' }}>{i + 1}</td>
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{
                        background: a.type === 'entree' ? '#e8f8ef' : '#fdeaea',
                        color: a.type === 'entree' ? '#1a7a40' : '#e84c3d',
                        padding: '2px 8px', borderRadius: '12px', fontSize: '11px', fontWeight: 600
                      }}>
                        {a.type === 'entree' ? '↑ Entrée' : '↓ Sortie'}
                      </span>
                    </td>
                    <td style={{ padding: '10px 14px', fontWeight: '500', color: '#2c2c2c' }}>{a.titre}</td>
                    <td style={{ padding: '10px 14px', color: '#555' }}>{a.service_nom || '—'}</td>
                    <td style={{ padding: '10px 14px', color: '#555' }}>{CATEGORIES.find(c => c.value === a.categorie)?.label || a.categorie}</td>
                    <td style={{ padding: '10px 14px', fontWeight: '600', color: a.type === 'entree' ? '#1a7a40' : '#e84c3d' }}>
                      {a.type === 'entree' ? '+' : '-'}{fmt(a.montant)} DH
                    </td>
                    <td style={{ padding: '10px 14px', color: '#555' }}>{a.date}</td>
                    <td style={{ padding: '10px 14px', color: '#555' }}>{a.personne || '—'}</td>
                    <td style={{ padding: '10px 14px' }}>
                      {a.photo_url ? (
                        <div onClick={() => setSelectedPhoto(a.photo_url)} style={{ cursor: 'pointer', display: 'inline-block' }}>
                          <img src={a.photo_url} alt="justificatif" style={{ width: '44px', height: '44px', objectFit: 'cover', borderRadius: '6px', border: '2px solid #e0e0e0' }}
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <select value={a.statut}
                        onChange={e => updateStatut(a.id, e.target.value, activeTab === 'principale' ? undefined : activeTab as number)}
                        style={{ padding: '3px 8px', borderRadius: '4px', fontSize: '11px', border: '1px solid #e0e0e0', cursor: 'pointer', background: a.statut === 'traitee' ? '#e8f8ef' : '#fff3e0', color: a.statut === 'traitee' ? '#1a7a40' : '#e65100' }}>
                        <option value="en_cours">En cours</option>
                        <option value="traitee">Traitée</option>
                      </select>
                    </td>
                    <td style={{ padding: '10px 14px', color: '#555' }}>{a.description || '—'}</td>
                    <td style={{ padding: '10px 14px' }}>
                      <button onClick={() => deleteAction(a.id, activeTab === 'principale' ? undefined : activeTab as number)}
                        style={{ background: '#fdeaea', color: '#e84c3d', border: '1px solid #f5c6c6', borderRadius: '4px', padding: '3px 8px', cursor: 'pointer', fontSize: '11px' }}>
                        🗑
                      </button>
                    </td>
                  </tr>
